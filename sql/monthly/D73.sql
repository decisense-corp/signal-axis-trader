-- ============================================================================
-- D73_two_axis_candidate_patterns_sell
-- スコア系2軸パターン候補抽出（SELL版）
-- 
-- 期間設定（ここを変更すれば全体に反映）
-- ============================================================================
...
-- 既存テーブル削除
DROP TABLE IF EXISTS `kabu-376213.kabu2411.D73_two_axis_candidate_patterns_sell`;

-- 新規作成
CREATE TABLE `kabu-376213.kabu2411.D73_two_axis_candidate_patterns_sell` AS
WITH 
-- 1. 2軸パターンの全組み合わせ生成
base_patterns AS (
  SELECT 
...
FROM `kabu-376213.kabu2411.D73_two_axis_candidate_patterns_sell`;
...
-- BUY/SELL比較用：同じパターンの比較
WITH buy_patterns AS (
  SELECT pattern_key, avg_profit_rate as buy_profit_rate, win_rate as buy_win_rate
  FROM `kabu-376213.kabu2411.D63_two_axis_candidate_patterns_buy`
),
sell_patterns AS (
  SELECT pattern_key, avg_profit_rate as sell_profit_rate, win_rate as sell_win_rate
  FROM `kabu-376213.kabu2411.D73_two_axis_candidate_patterns_sell`
)
SELECT 
  COALESCE(b.pattern_key, s.pattern_key) as pattern_key,
  b.buy_profit_rate,
  s.sell_profit_rate,
  b.buy_win_rate,
  s.sell_win_rate,
  ROUND(COALESCE(b.buy_profit_rate, 0) + COALESCE(s.sell_profit_rate, 0), 4) as total_profit_rate
FROM buy_patterns b
FULL OUTER JOIN sell_patterns s ON b.pattern_key = s.pattern_key
WHERE b.pattern_key IS NOT NULL AND s.pattern_key IS NOT NULL
