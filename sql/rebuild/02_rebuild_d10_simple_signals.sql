-- ============================================================================
-- Phase 2: d10_simple_signals テーブル再構築（シンプル版）
-- ============================================================================

-- 1. バックアップ
CREATE TABLE IF NOT EXISTS `kabu-376213.kabu2411.d10_simple_signals_backup_20250702` AS
...
OPTIONS(
  description="Daily signal calculation values history table (new system). 33 types of signals.",
...
WITH quotes_data AS (
  SELECT 
    ...
    LEAD(dq.Date) OVER (PARTITION BY REGEXP_REPLACE(dq.Code, '0$', '') ORDER BY dq.Date) as signal_date,
    dq.Open,
    dq.High,
    dq.Low,
    dq.Close,
...
-- 4. 結果確認
SELECT 
  'Data insertion completed' as message,
